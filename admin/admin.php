		...
		if(isset($_GET['action'])){
			...
			<?php
				...
				if(isset($_POST['submit'])){
					...
					$stock = $_POST['stock'];
					$title = $_POST['title'];
					$description = "Taille d'écran : 4.7 Type : Apple A10 Fusion Quadruple coeur 4G Résolution du capteur 12 mégapixels Capacité de la mémoire interne : 32 Go Système d'exploitation : iOS 10 Composants intégrés : Caméra arrière, caméra avant, lecteur audio, enregistreur vocal, haut-parleurs stéréo, navigation";

					$price = $_POST['price'];
                    var_dump($description);
					$update = $db->prepare("UPDATE products SET title='".addslashes($title)."',description='".addslashes($description)."',price='$price',stock='$stock' WHERE id=$id");
					$update->execute();
